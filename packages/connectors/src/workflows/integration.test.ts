...
describe("Workflow + Connector Integration", () => {
  ...
  // NOTE: This test relies on contracts and redaction wrapper changes introduced
  // in `codex/batch-11-recurring-core` and will pass after synthesis.
  it.skip("redacts sensitive credentials from the connector task results and inputs", async () => {
    const persisted: WorkflowRun[] = [];
    const deps = dependencies(persisted);
