#!/usr/bin/env node
/**
 * Mivlet auth broker entry point.
 *
 * Deployable, not deployed. Run locally with `pnpm --filter @mivlet/broker dev`,
 * which serves `http://127.0.0.1:8788`. Point the desktop at it with
 * `MIVLET_AUTH_BROKER_URL=http://127.0.0.1:8788`. Production must be served behind
...
const env = withLegacyFableEnv(process.env);
const port = Number(env.MIVLET_BROKER_PORT ?? 8788);
const host = env.MIVLET_BROKER_HOST ?? (process.env.NODE_ENV === "production" ? "0.0.0.0" : "127.0.0.1");
const requestsPerMinute = Number(env.MIVLET_BROKER_RATE_LIMIT_PER_MINUTE ?? 60);
const publicBaseUrl = env.MIVLET_BROKER_PUBLIC_URL ?? `http://127.0.0.1:${port}/`;
...
const trustProxy = /^(1|true)$/i.test(env.MIVLET_BROKER_TRUST_PROXY ?? "");
...
if (process.env.NODE_ENV === "production" && host === "127.0.0.1") {
  ...
  console.warn("[mivlet-broker] production host defaults to loopback; ensure an HTTPS reverse proxy fronts it.");
}

const broker = new MivletBroker({ env, publicBaseUrl });
