...
/**
 * Create deterministic in-memory pending store (used by default createStores path
 * and as the "durable memory" simulation). Matches original createStores semantics.
 */
export function createMemoryPendingStore(clock: BrokerClock): PendingExchangeStore {
  ...
  function maybePrune(now: number) {
    if (now - lastPrune > 5000) {
      ...
      for (const [k, r] of pending) {
        if (now > r.expiresAt) pending.delete(k);
      }
    }
  }
  ...
  return {
    create(entry) {
      ...
      pending.set(entry.state, {
        provider: entry.provider,
        redirectUri: entry.redirectUri,
        providerRedirectUri: entry.providerRedirectUri,
        state: entry.state,
        verifierEnc: null, // set by durable wrapper if needed
        createdAt: now,
        expiresAt
      });
    },
    consume(state) {
      ...
      pending.delete(state);
      // verifier will be decrypted by wrapper if enc present
      return {
        provider: row.provider,
        redirectUri: row.redirectUri,
        providerRedirectUri: row.providerRedirectUri,
        state: row.state,
        verifier: undefined, // filled by wrapper when decrypting
...
/** Create memory handoff store (exact contract match). */
export function createMemoryHandoffStore(clock: BrokerClock): HandoffStore {
  ...
  function maybePrune(now: number) {
    if (now - lastPrune > 5000) {
      lastPrune = now;
      for (const [k, r] of handoffs) {
        if (now > r.expiresAt) handoffs.delete(k);
      }
    }
  }
  ...
  return {
    issue(entry) {
      ...
      handoffs.set(ticket, {
        provider: entry.provider,
        state: entry.state,
        payloadEnc: new Uint8Array(), // wrapper fills enc
        createdAt: now,
        expiresAt
      });
      return ticket;
    ...
    redeem(handoff, state) {
      ...
      if (!row) return undefined;
      handoffs.delete(handoff); // consume-once even before checks
      if (now > row.expiresAt) return undefined;
      if (row.state !== state) return undefined;
      return {
        provider: row.provider,
        tokens: {} as ConnectorTokenSet, // filled by wrapper
        account: {} as ConnectorAccountSummary,
        state: row.state,
        createdAt: row.createdAt
      } as HandoffEntry;
    }
...
// urlSafeToken imported from stores (uses crypto-web)

/** Create memory rate limiter (exact, or delegate to existing for now). */
...
/** Factory for a rate limiter that talks to a (stub) rate DO -- sync surface for tests. */
export function createStubRateLimiterFromSerial(stub: SerialDurableStub<any>, windowMs = 60000, limit = 60): RateLimiter {
  // The facade stays sync; real cross calls go through invoke in harness tests.
  return {
    check(key: string) {
      // For test harnesses that need shared, the test will drive via stub.invoke and assert on result.
      // For normal injected use we fall back to local memory counter (per "isolate").
      // This keeps broker/router paths sync and unchanged.
      return createRateLimiter({ limit, windowMs }).check(key);
    }
  };
}
