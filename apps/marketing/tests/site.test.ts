...
describe("marketing site build artifacts (M-*)", () => {
  it("M-01 home has the light product-led hero and provider section", () => {
    ...
    if (!existsSync(p)) {
      console.warn("dist not present; run build first");
      return;
    }
    ...
    expect(html).toMatch(/OpenAI API/i);
    expect(html).toMatch(/Claude/i);
    expect(html).toMatch(/Gemini/i);
    expect(html).toMatch(/>xAI</i);
    expect(html).toMatch(/Custom API/i);
    expect(html).not.toMatch(/>Codex</i);
    expect(html).not.toMatch(/OpenCode|DeepSeek|MiniMax|Kimi|Z\.AI/i);
  ...
  it("M-02 connectors uses only allowed status labels", () => {
    ...
    expect(html).toMatch(
      /Implemented|Functional but gated|Preview-only|Missing/,
    );
