...
describe("mission runtime boundary", () => {
  ...
  it("composes only the authenticated native mission commands", async () => {
    ...
    const start = { runId: "run-1", workerId: "worker-1", runStartEventId: "event-run-start", workerStartedEventId: "event-worker-start", idempotencyKey: "start-1", expectedRunRevision: 3, expectedLastSequence: 2 };
