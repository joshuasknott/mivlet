...
export function ScheduleEditor({ runtime, schedule, pending, onSave, onCancel, initialAgentId }: { runtime: SettingsRuntime; project?: { id: string; name: string; participantIds: string[] }; initialAgentId?: string; schedule: LocalSchedule | null; pending: boolean; onSave: (input: EditorInput) => void; onCancel: () => void }) {
  ...
  const timezones = useMemo(() => [...new Set(["UTC", Intl.DateTimeFormat().resolvedOptions().timeZone, ...Intl.supportedValuesOf("timeZone")])], []);
  const [effort, setEffort] = useState<string | undefined>(schedule?.reasoningEffort);
  ...
  const trigger = useMemo<LocalScheduleTrigger>(() => kind === "once" ? { kind, localDateTime: dateTime } : kind === "daily" ? { kind, localTime: time } : { kind, weekday, localTime: time }, [kind, dateTime, time, weekday]);
  const preview = useQuery({ queryKey: ["local-schedule-preview", timezone, trigger], queryFn: () => previewLocalSchedule({ timezone, trigger }), retry: false, refetchInterval: 30_000, enabled: Boolean(timezone && (kind === "once" ? dateTime : time)) });
  ...
  return <form className="profile-section local-schedules__editor" onSubmit={(event) => {
    ...
    {preview.error ? <p role="alert">{errorText(preview.error)}</p> : preview.data ? <p role="status">Next run: <time dateTime={preview.data}>{new Date(preview.data).toLocaleString([], { timeZone: timezone, dateStyle: "full", timeStyle: "short" })}</time> · {timezone}</p> : <p role="status">{preview.isFetching ? "Calculating next run…" : "Choose a date and time to preview the next run."}</p>}
    {invalid ? <p role="alert">{invalid}</p> : null}
    <div className="profile-action-row"><button type="submit" className="button button--primary" disabled={pending || !supported || !effortValid || !prompt.trim() || !preview.data || !!preview.error}>{pending ? "Saving…" : "Save schedule"}</button><button type="button" className="button button--secondary" disabled={pending} onClick={onCancel}>Close</button></div>
  </form>;
}
