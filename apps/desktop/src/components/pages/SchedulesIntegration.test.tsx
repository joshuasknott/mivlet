...
/**
 * Schedules + Run History UI Integration Tests
 *
 * NOTE: This test file requires synthesis (merging with `codex/batch-11-schedule-ui`
 * and `codex/batch-11-run-history-ui`) before it can compile and run.
 * The baseline branch lacks the `RunHistoryPage` component and updated `SchedulePanel`
 * properties.
 *
 * Once synthesized, uncomment the code below to run these integration tests.
 */

describe("Schedules UI Integration (Pending Synthesis)", () => {
  it("requires synthesis before executing UI integration tests", () => {
...
describe("Schedules + Run History UI Integration", () => {
  ...
  it("integrates RunHistoryPage navigation, viewing failed run detail, and triggering retry", async () => {
    ...
    expect(screen.getByText("Backup server timed out.")).toBeInTheDocument();
