...
describe("Work details", () => {
  ...
  it("records deliberate steering at the current generation", async () => {
    const onSteer = vi.fn();
    ...
    await act(async () => { fireEvent.change(screen.getByPlaceholderText(/safe boundary/), { target: { value: "Add the conclusion" } }); });
    await act(async () => { fireEvent.click(screen.getByRole("button", { name: "Apply steering" })); });
    expect(onSteer).toHaveBeenCalledWith("work", 2, "Add the conclusion");
  });

  it("requires explicit reconciliation before continuing started work", async () => {
  ...
  it("promotes an explicitly selected conclusion through the memory callback", async () => {
    ...
    await act(async () => { fireEvent.click(screen.getByRole("button", { name: "Save to memory…" })); });
    const editor = screen.getByLabelText(/Conclusion to keep/) as HTMLTextAreaElement;
    expect(editor.value).toContain("A saved report.");
    await act(async () => { fireEvent.change(editor, { target: { value: "Keep the summary short." } }); });
    const form = document.querySelector("form.work-promote-form");
    if (!form) throw new Error("Expected the memory promotion form.");
    await act(async () => { fireEvent.click(within(form as HTMLElement).getByRole("button", { name: "Save to memory" })); });
  ...
  it("keeps the conclusion editor when promotion fails", async () => {
    ...
    render(<WorkDetails item={item()} onOpen={vi.fn()} onStop={vi.fn()} onContinue={vi.fn()} onSteer={vi.fn()} onPromote={onPromote} />);
    await act(async () => { fireEvent.click(screen.getByRole("button", { name: "Save to memory…" })); });
    await act(async () => { fireEvent.change(screen.getByLabelText(/Conclusion to keep/), {
