...
export function PluginPanel({
  ...
  return (
    <section className="connectors-marketplace" aria-label="Plugins">
      ...
      {showBuiltins ? (
        <section className="marketplace-section" aria-labelledby="marketplace-section-built-in">
          ...
          {builtin.plugins === null ? (
            <p className="marketplace-section__empty">
              {workspaceId
                ? "Built-in plugin settings are unavailable. Open the desktop app to manage them."
                : "Open the desktop app to manage built-in plugins."}
            </p>
          ) : null}
          {builtin.loadError ? <p role="status" className="marketplace-section__empty">{builtin.loadError}</p> : null}
        </section>
      ) : null}
...
function ConnectorDetails({
  ...
  return <article className="connector-detail" aria-label={`${connector.name} details`} aria-busy={busy}>
    ...
    <div className="connector-detail__header">
      ...
      <span className={`connector-detail__status connector-detail__status--${detail.className}`}>{busy ? "Connecting…" : detail.label}</span>
    </div>
    ...
    <p className="connector-detail__intro">{ready ? "Connected. Your agents can use the access you granted." : configured ? `Sign in to use ${connector.name} in your conversations.` : "This connection is not available on this installation yet."}</p>
    <div className="connector-detail__actions">
...
export function resolveDetailedStatus(connector: ConnectorManifest): {
  ...
  // Default: not yet connected, authorization still required.
  return {
    label: "Needs attention",
