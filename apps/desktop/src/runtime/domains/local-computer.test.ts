...
function native(value: boolean) {
  selectRuntimeAdapterForTest(value ? "native" : "preview");
}
beforeEach(() => {
  vi.resetAllMocks();
  native(false);
...
describe("native Windows runtime boundary", () => {
  it("does not simulate computer authority in a browser preview", async () => {
    ...
    await expect(stopRuntimeAppControl()).resolves.toBeNull();
  ...
  it("keeps files explicitly workspace scoped and propagates native prerequisite errors", async () => {
    ...
    expect(mocks.invoke).toHaveBeenCalledWith("local_computer_file_preview", {
      request: { ...target, path: "notes/plan.md" },
    });
    mocks.invoke.mockRejectedValueOnce({
      message: "The bundled runtime checksum does not match.",
    });
    ...
    mocks.invoke.mockResolvedValue([
      {
        ...
        originalName: "totals.csv",
        mimeType: "text/csv",
        relativePath: "Attachments/upload-a/totals-a.csv",
        sizeBytes: 4,
        sha256: "hash",
        stagedAt: "now",
      },
  ...
  it("discards one exact unadopted attachment batch", async () => {
    ...
    expect(mocks.invoke).toHaveBeenCalledWith(
      "local_computer_discard_attachment_batch",
      { request },
    );
  });
});
