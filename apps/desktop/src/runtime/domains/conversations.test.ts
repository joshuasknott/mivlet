...
import { loadRuntimeConversationDraft } from "./conversations";
import {
...
describe("native conversation drafts", () => {
  ...
  it("restores a new-conversation draft with a native null thread ID", async () => {
    ...
    await expect(loadRuntimeConversationDraft("new-thread")).resolves.toEqual({
      ...draft,
      threadId: undefined,
    });
  });
  ...
  it.each([
    ...
    await expect(loadRuntimeConversationDraft("new-thread")).rejects.toThrow(
      "Malformed or cross-workspace",
    );
  });
  ...
  it("keeps a missing draft distinct from invalid data", async () => {
    ...
    await expect(
      loadRuntimeConversationDraft("new-thread"),
    ).resolves.toBeNull();
  });
  ...
  it("binds native draft reads to the expected workspace and exact thread", async () => {
    ...
    ).rejects.toThrow("cross-conversation");
  });
});
