...
describe("hosted computer runtime boundary", () => {
  ...
  it("does not simulate cloud computers in browser preview", async () => {
    await expect(
      loadRuntimeHostedComputer("workspace-a", "agent-a"),
    ).resolves.toBeNull();
    await expect(
      provisionRuntimeHostedComputer("workspace-a", "agent-a", "device-a"),
    ).resolves.toBeNull();
    await expect(
      prepareRuntimeHostedProcess({
  ...
  it("uses fixed native commands and passes only public coordination ids", async () => {
    ...
    expect(mocks.invoke.mock.calls).toEqual([
      [
        "hosted_computer_status",
        { workspaceId: "workspace-a", agentId: "agent-a" },
      ],
      [
        "hosted_computer_provision",
        {
          workspaceId: "workspace-a",
          agentId: "agent-a",
          deviceId: "device-a",
  ...
  it("keeps hosted capabilities behind fixed native process commands", async () => {
    ...
    expect(mocks.invoke.mock.calls).toEqual([
      ...
      [
        "hosted_process_launch",
        { request: { proposal, resolution, sourceResolution: resolution } },
      ],
