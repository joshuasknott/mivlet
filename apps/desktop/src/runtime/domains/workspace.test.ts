...
import { loadRuntimeSnapshot, saveRuntimeSnapshot } from "./workspace";
import {
...
describe("native snapshot persistence", () => {
  ...
  it("distinguishes a failed read from a missing snapshot", async () => {
    ...
    mocks.invoke
      .mockRejectedValueOnce("Temporary disk read failure")
      .mockResolvedValueOnce(null);
    await expect(loadRuntimeSnapshot()).rejects.toThrow(
      "Temporary disk read failure",
    );
