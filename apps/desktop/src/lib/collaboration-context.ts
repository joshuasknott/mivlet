...
export function collaborationContext(
  ...
  const parts = [
    ...
    "When coordinating, discover workspace-agents, break the request into distinct useful assignments, and use dependencies for work that needs another result. Independent assignments may run together. If the user asks for all agents, give each a relevant contribution within the remaining budget; explain any unavailable participants or limit instead of duplicating work or claiming they participated. Workers may consult another suitable agent. Read the saved results, resolve questions and failures, and synthesize only what the returned evidence supports.",
    `This effort belongs to conversation ${room.id}. Its existing participants are ${boundedJson(room.participants, 2500)}. Only the selected originating context is shared; unrelated private conversation histories are never available. Workspace agents can join this effort without joining a project. Use teammate-assign for a relevant question, challenge, review or concrete task. Participants run their own selected models. The call returns an ID, not their answer. Finish your contribution promptly after dispatch; Mivlet resumes you when results arrive. Return results to the requester by finishing your response; do not hand back cyclically. You may assign at most four children, to depth two. Use teammate-message for a task-scoped question or follow-up to an existing assignment; a question to a waiting lead must not become a new dependency on that lead. Stop when the request is answered; use team-await-user for a specific user decision, missing prerequisite or uncertain external outcome.`,
