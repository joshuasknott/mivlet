...
const membership = (
  over: Partial<AccountWorkspaceSummary> & Pick<AccountWorkspaceSummary, "fableWorkspaceId">
): AccountWorkspaceSummary => ({
...
const status = (over: Partial<AccountWorkspaceStatus> = {}): AccountWorkspaceStatus => ({
  configured: true,
  state: "ready",
  message: "Ready.",
  accountBound: true,
...
describe("hosted computer scope", () => {
  it("uses the only active membership and device without list-order search", () => {
    ...
    expect(resolveHostedWorkspaceId(workspaces, status().activeWorkspace)).toBe("workspace-zeta");
  ...
  it("does not use the first mirrored membership when several are active", () => {
    ...
    expect(resolveHostedWorkspaceId(workspaces, status().activeWorkspace)).toBeNull();
    expect(resolveHostedComputerScope(status({
      workspaces,
  ...
  it("uses an explicit selection that matches the local active context", () => {
    ...
    expect(resolveHostedWorkspaceId(workspaces, {
      localWorkspaceId: "local-beta",
      fableWorkspaceId: "workspace-beta",
      name: "Beta",
      source: "hosted"
