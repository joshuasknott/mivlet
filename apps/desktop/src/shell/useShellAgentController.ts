...
export function useShellAgentController({ onDictation, onVoiceCancel }: { onDictation: (transcript: string) => void; onVoiceCancel: () => void }) {
  ...
  const agent = useNativeAgent({ providers: runtime.backendProviders, activeProviderId: runtime.connectedAgentBackend?.id, models: runtime.selectableModels, threadId: runtime.activeThread?.id ?? runtime.activeItem, execute: executor, shouldCancel: () => cancelRequestedRef.current, onCancel: () => { cancelRequestedRef.current = true; cancelApprovals(); }, onToolCall: queueToolApproval });
  ...
  return { runtime, agent, voice, scheduledActive: scheduledAgent.active, resetCancellation: () => { cancelRequestedRef.current = false; } };
}
