...
export async function startRuntimeEmbeddedMcp(requestId: string) {
  if (!hasTauriRuntime()) throw new Error("MCP discovery requires the desktop app.");
  await invoke("start_embedded_mcp", { requestId });
}
export async function sendRuntimeEmbeddedMcp(requestId: string, frame: unknown) {
...
export async function listenRuntimeEmbeddedMcp(requestId: string, receive: (frame: unknown) => void) {
  if (!hasTauriRuntime()) return null;
  return listen<unknown>(`fable://embedded-mcp/${requestId}`, event => receive(event.payload));
}
...
export async function startRuntimeEmbeddedAgent(input: {
  ...
  if (!hasTauriRuntime()) throw new Error("The native agent host is unavailable.");
  await invoke("start_embedded_agent", { input });
}
export async function replyRuntimeEmbeddedAgent(requestId: string, callId: string, ok: boolean, output: string) {
...
export async function listenRuntimeEmbeddedAgent(requestId: string, receive: (event: EmbeddedRuntimeEvent) => void) {
  if (!hasTauriRuntime()) return null;
  return listen<EmbeddedRuntimeEvent>(`fable://embedded-agent/${requestId}`, event => receive(event.payload));
}
export {
