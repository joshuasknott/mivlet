...
use crate::store::repos::{
    connector_account, knowledge_source, memory_record, preferences, schedule,
    workflow, workspace, scheduled_job, scheduler_queue,
};
use crate::store::vault::{MasterKey, Vault};
use crate::store::{Store, StoreError};
use crate::portable::{
    export_workspace, import_workspace, PORTABLE_FORMAT_NAME,
    PORTABLE_FORMAT_VERSION, ImportOptions,
};
...
fn test_workspace_isolation_crud_operations() {
    ...
    store.transaction(|tx| {
        preferences::upsert_scoped(tx, &store, &alpha_scope, "theme", &json!({"color": "red"}), "now")?;
        preferences::upsert_scoped(tx, &store, &beta_scope, "theme", &json!({"color": "blue"}), "now")?;
        Ok(())
    }).unwrap();

    let alpha_pref = store.with_conn(|conn| preferences::get_scoped(conn, &store, &alpha_scope, "theme")).unwrap().unwrap();
    let beta_pref = store.with_conn(|conn| preferences::get_scoped(conn, &store, &beta_scope, "theme")).unwrap().unwrap();
    ...
    assert!(store.transaction(|tx| {
        preferences::upsert_scoped(tx, &store, &alpha_proj_scope, "theme", &json!({"color": "green"}), "now")
    }).is_err());

    // 3. Connector Isolation
    store.transaction(|tx| {
        connector_account::upsert_from_value_scoped(tx, &store, &alpha_scope, json!({
            "connectorId": "slack",
            "status": "connected",
            "credentialRef": "oauth-token:slack:1"
        }), "now")?;
        connector_account::upsert_from_value_scoped(tx, &store, &beta_scope, json!({
            "connectorId": "slack",
            "status": "disconnected",
    ...
    let alpha_connectors = store.with_conn(|conn| connector_account::list_scoped(conn, &store, &alpha_scope)).unwrap();
    let beta_connectors = store.with_conn(|conn| connector_account::list_scoped(conn, &store, &beta_scope)).unwrap();
    ...
    let alpha_k = store.with_conn(|conn| knowledge_source::list_scoped(conn, &store, &alpha_scope)).unwrap();
    let beta_k = store.with_conn(|conn| knowledge_source::list_scoped(conn, &store, &beta_scope)).unwrap();
    assert_eq!(alpha_k.len(), 1);
    assert_eq!(beta_k.len(), 1);
    assert_eq!(alpha_k[0].id, "k1_alpha");
    assert_eq!(beta_k[0].id, "k1_beta");

    let alpha_m = store.with_conn(|conn| memory_record::list_scoped(conn, &store, &alpha_scope)).unwrap();
    let beta_m = store.with_conn(|conn| memory_record::list_scoped(conn, &store, &beta_scope)).unwrap();
    assert!(alpha_m[0].approved);
    assert!(!beta_m[0].approved);

    let alpha_s = store.with_conn(|conn| schedule::list_scoped(conn, &store, &alpha_scope)).unwrap();
    let beta_s = store.with_conn(|conn| schedule::list_scoped(conn, &store, &beta_scope)).unwrap();
    assert_eq!(alpha_s[0].weekday, "Mon");
    assert_eq!(beta_s[0].weekday, "Tue");

    let alpha_wf = store.with_conn(|conn| workflow::list_definitions(conn, &store, &alpha_scope)).unwrap();
    let beta_wf = store.with_conn(|conn| workflow::list_definitions(conn, &store, &beta_scope)).unwrap();
    assert_eq!(alpha_wf[0]["step"], 1);
    assert_eq!(beta_wf[0]["step"], 2);

    let alpha_run = store.with_conn(|conn| workflow::list_runs(conn, &store, &alpha_scope, None)).unwrap();
    let beta_run = store.with_conn(|conn| workflow::list_runs(conn, &store, &beta_scope, None)).unwrap();
    ...
    store.transaction(|tx| {
        knowledge_source::delete_scoped(tx, &beta_scope, "k1_beta")
    }).unwrap();
    let alpha_k_post = store.with_conn(|conn| knowledge_source::list_scoped(conn, &store, &alpha_scope)).unwrap();
    let beta_k_post = store.with_conn(|conn| knowledge_source::list_scoped(conn, &store, &beta_scope)).unwrap_or_default();
    ...
    assert!(store.transaction(|tx| {
        preferences::upsert_scoped(tx, &store, &invalid_workspace_scope, "theme", &json!({"color": "red"}), "now")
    }).is_err());
    ...
    assert!(store.with_conn(|conn| switched_scope.ensure_exists(conn)).is_err());
}
...
fn test_project_scoped_versus_workspace_only_queries() {
    ...
    let wk_list = store.with_conn(|conn| knowledge_source::list_scoped(conn, &store, &alpha)).unwrap();
    assert_eq!(wk_list.len(), 1);
    assert_eq!(wk_list[0].id, "wk1");

    // Project-level query should ONLY return project-scoped row
    let pk_list = store.with_conn(|conn| knowledge_source::list_scoped(conn, &store, &proj_a_scope)).unwrap();
...
fn test_prevention_of_cross_workspace_export() {
    ...
    store.transaction(|tx| {
        preferences::upsert_scoped(tx, &store, &default_scope, "key1", &json!("def-val"), "now")?;
        preferences::upsert_scoped(tx, &store, &alpha_scope, "key1", &json!("alpha-val"), "now")?;
        Ok(())
    }).unwrap();
...
fn test_legacy_json_to_encrypted_sqlite_success_and_idempotency() {
    ...
    fs::write(dir.path().join("runtime-snapshot.json"), serde_json::to_vec(&snapshot_data).unwrap()).unwrap();
    ...
    fs::write(dir.path().join("scheduler-store.json"), serde_json::to_vec(&scheduler_data).unwrap()).unwrap();

    // 2. Perform Migration
    crate::store::migrations::migrate_all(&store, dir.path()).unwrap();

    // 3. Assert rows exist in SQLite
    let (active_item, voice_enabled): (String, bool) = store.with_conn(|conn| {
        let prefs = preferences::get_scoped(conn, &store, &DataScope::legacy_default(), "shell")?.unwrap();
        Ok((
    ...
    let jobs = store.with_conn(|conn| scheduled_job::list(conn, &store, "default")).unwrap();
    let queue = store.with_conn(|conn| scheduler_queue::list(conn, &store, "default")).unwrap();
    ...
    let jobs_post = store.with_conn(|conn| scheduled_job::list(conn, &store, "default")).unwrap();
...
fn test_migration_tolerates_malformed_json_and_rolls_back_partially_written_source() {
    ...
    let keys = store.with_conn(|conn| preferences::keys_scoped(conn, &DataScope::legacy_default())).unwrap();
    ...
    fs::write(dir.path().join("scheduler-store.json"), serde_json::to_vec(&invalid_scheduler_data).unwrap()).unwrap();
...
fn test_encryption_at_rest_safety() {
    ...
    // Insert preferences
    store.transaction(|tx| {
        preferences::upsert_scoped(tx, &store, &scope, "secret_key", &json!({"secret": "super_confidential"}), "now")
    }).unwrap();

    // Open connection and inspect raw database (ciphertext check)
    let (ciphertext, nonce): (Vec<u8>, Vec<u8>) = store.with_conn(|conn| {
        conn.query_row(
    ...
    let sealed = crate::store::vault::Sealed { ciphertext: ciphertext.clone(), nonce: nonce.clone() };
    let opened = store.open_payload(&sealed, "preferences:default:secret_key").unwrap();
    ...
    assert!(store.open_payload(&sealed, "preferences:default:wrong_key").is_err());
}
...
fn test_export_import_roundtrip_and_validation() {
    ...
    store.transaction(|tx| {
        ...
        connector_account::upsert_from_value_scoped(tx, &store, &scope, json!({
            "connectorId": "gmail",
            "status": "connected",
            "credentialRef": "oauth-token:gmail:ref"
        }), "now")?;
        schedule::upsert_from_value_scoped(tx, &store, &scope, json!({
    ...
    // 2. Export manifest
    let manifest = export_workspace(&store).unwrap();
    assert_eq!(manifest.sections.preferences.len(), 1, "preferences must contain 1 exported row");
    assert_eq!(manifest.sections.connector_accounts.len(), 1, "connector accounts must contain 1 exported row");
    assert_eq!(manifest.sections.schedules.len(), 1, "schedules must contain 1 exported row");
    ...
    let imported_pref = store2.with_conn(|conn| preferences::get_scoped(conn, &store2, &scope, "pref1")).unwrap().unwrap();
    assert_eq!(imported_pref, json!("val1"));

    // Imported connector is written 'disconnected' and lacks credential_ref
    let imported_connectors = store2.with_conn(|conn| connector_account::list_scoped(conn, &store2, &scope)).unwrap();
    ...
    let imported_schedules = store2.with_conn(|conn| schedule::list_scoped(conn, &store2, &scope)).unwrap();
...
fn test_import_failure_rolls_back_atomically() {
    ...
    let prefs = store.with_conn(|conn| preferences::keys_scoped(conn, &DataScope::workspace("default").unwrap())).unwrap();
...
fn test_dlp_workspace_deletion_cascade_isolation() {
    ...
    store.transaction(|tx| {
        tx.execute("DELETE FROM workspace WHERE id='alpha';", []).map_err(StoreError::from)?;
        Ok(())
    }).unwrap();

    // Verify alpha knowledge source is deleted
    let alpha_list = store.with_conn(|conn| knowledge_source::list_scoped(conn, &store, &alpha)).unwrap_or_default();
    assert_eq!(alpha_list.len(), 0);

    // Verify beta knowledge source is untouched
    let beta_list = store.with_conn(|conn| knowledge_source::list_scoped(conn, &store, &beta)).unwrap();
...
fn test_dlp_failed_import_preserves_existing_data() {
    ...
    store.transaction(|tx| {
        preferences::upsert_scoped(tx, &store, &scope, "existing_key", &json!("pre-import-val"), "now")
    }).unwrap();
    ...
    let val = store.with_conn(|conn| preferences::get_scoped(conn, &store, &scope, "existing_key")).unwrap().unwrap();
...
fn test_dlp_backup_lifecycle_checks() {
    ...
    conn.execute_batch("CREATE TABLE test (id TEXT PRIMARY KEY); INSERT INTO test VALUES ('1');").unwrap();

    // 1. Vacuum backup to new path
    let backup_path = dir.path().join("backup.db");
    conn.execute("VACUUM INTO ?1", [backup_path.to_string_lossy().as_ref()]).unwrap();
    assert!(backup_path.exists());

    // Verify backup contains the table and data
    let backup_conn = Connection::open(&backup_path).unwrap();
    let count: i64 = backup_conn.query_row("SELECT COUNT(*) FROM test;", [], |r| r.get(0)).unwrap();
    assert_eq!(count, 1);

    // 2. Vacuum backup to existing path should fail (standard sqlite behavior is overwrite block or error depending on mode)
    assert!(conn.execute("VACUUM INTO ?1", [backup_path.to_string_lossy().as_ref()]).is_err());
}
