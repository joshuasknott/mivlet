...
mod tests {
    ...
    #[test]
    fn project_grant_uses_workspace_connection_without_widening_scope() {
        let (store, workspace_scope, connection_id) = setup();
        let project_scope = store
            .transaction(|tx| {
                ...
                resolve(
                    tx,
                    Some(workspace_scope.data.workspace_id()),
                    Some("project-a"),
                    ScopeAccess::Write,
                )
            })
            ...
            .transaction(|tx| {
                create(
                    ...
                    CreateCapabilityGrant {
                        ...
                        max_uses: None,
                        expires_at: None,
                        granted_at: "2026-07-11T20:01:00Z",
                    },
                )
            })
            .unwrap();
        assert_eq!(grant.scope_kind, "project");
        assert_eq!(grant.project_id.as_deref(), Some("project-a"));
        assert!(store
            .with_conn(|tx| {
                check(
                    tx,
