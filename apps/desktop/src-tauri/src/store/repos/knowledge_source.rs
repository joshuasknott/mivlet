...
pub fn upsert_from_value(tx: &Connection, store: &Store, value: Value, now: &str) -> Result<()> {
    ...
    let sealed = seal_json(store, &value, &format!("legacy_unowned:knowledge:{id}"))?;
    tx.execute(
        "INSERT OR REPLACE INTO private_context_legacy_unowned
         ...
         VALUES ('knowledge','default',?1,NULL,?2,?3,?4,'legacy import had no authenticated owner')",
...
pub fn upsert_from_value_scoped(
    ...
    let private = PrivateDataScope::legacy_unowned(scope.clone());
    upsert_private(tx, store, &private, value, now)
}
...
pub fn upsert_private(
    ...
    let tombstoned: bool = tx.query_row(
        "SELECT EXISTS(SELECT 1 FROM knowledge_tombstone WHERE workspace_id=?1 AND owner_subject=?2 AND id=?3);",
    ...
    tx.execute(
        "INSERT INTO knowledge_source (workspace_id, owner_subject, authority, visibility, owner_member_id, id, project_id, connector_id,
...
pub fn delete_private(tx: &Connection, scope: &PrivateDataScope, id: &str) -> Result<()> {
    ...
    let exists = tx
        .query_row(
            "SELECT 1 FROM knowledge_source WHERE workspace_id=?1 AND owner_subject=?2 AND id=?3 AND project_id IS ?4;",
    ...
    tx.execute(
        "INSERT OR REPLACE INTO knowledge_tombstone (workspace_id, owner_subject, id, deleted_at) VALUES (?1, ?2, ?3, ?4);",
...
fn aad(workspace_id: &str, owner_subject: &str, id: &str) -> String {
    format!("knowledge_source:{workspace_id}:{owner_subject}:{id}")
}
...
fn canonicalize_authority(value: &mut Value, scope: &PrivateDataScope) {
    let authority = serde_json::json!({
        ...
        "ownerMemberId": scope.owner_member_id(),
    ...
    if let Some(object) = value.as_object_mut() {
        ...
        object.insert("authorityScope".into(), authority);
        object.insert(
            "scope".into(),
