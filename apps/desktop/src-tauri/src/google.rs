...
async fn send_json(
    ...
    let (_, bytes) = send(connector_id, tokens, method, url, body).await?;
    if bytes.is_empty() {
...
pub(crate) async fn search(
    ...
    let (items, next_cursor) = match connector_id.as_str() {
        "google-drive" => drive_search(&tokens, &query, limit, request.cursor.as_deref()).await?,
        "gmail" => gmail_search(&tokens, &query, limit, request.cursor.as_deref()).await?,
        "google-calendar" => {
            calendar_search(&tokens, &query, limit, request.cursor.as_deref()).await?
        }
        _ => {
            return Err(error(
                &connector_id,
...
async fn drive_search(
    ...
    let response = send_json("google-drive", tokens, Method::GET, url, None).await?;
    let items = response
        .get("files")
        .and_then(Value::as_array)
        .ok_or_else(|| {
...
async fn gmail_search(
    ...
    let response = send_json("gmail", tokens, Method::GET, url, None).await?;
    let ids = response
        .get("messages")
        .and_then(Value::as_array)
        .cloned()
    ...
    for message in ids {
        let id = string(&message, "id").ok_or_else(|| {
            error(
                "gmail",
                "provider-unavailable",
                "Gmail search item has no id.",
                false,
            )
        })?;
        ...
        let value = send_json("gmail", tokens, Method::GET, detail, None).await?;
        items.push(gmail_item(&value)?);
    }
    Ok((items, string(&response, "nextPageToken")))
}
...
async fn calendar_search(
    ...
    if query.is_empty() {
        ...
        let response = send_json("google-calendar", tokens, Method::GET, url, None).await?;
        let items = response
            .get("items")
            .and_then(Value::as_array)
            .cloned()
...
pub(crate) async fn import(
    ...
    let preview = match connector_id.as_str() {
        "google-drive" => drive_content(&tokens, &request.item).await?,
        "gmail" => gmail_thread_content(&tokens, &request.item).await?,
        "google-calendar" => calendar_content(&tokens, &request.item).await?,
        _ => {
            return Err(error(
                &connector_id,
                "configuration-required",
...
async fn drive_content(
    ...
    let (_, bytes) = send("google-drive", tokens, Method::GET, url.clone(), None).await?;
...
fn decode_gmail_part(value: &Value, output: &mut Vec<String>, attachments: &mut Vec<String>) {
    ...
    if let Some(parts) = value.get("parts").and_then(Value::as_array) {
        for part in parts {
            decode_gmail_part(part, output, attachments);
        }
    }
}
...
async fn gmail_thread_content(
    ...
    for message in messages {
        ...
        if let Some(payload) = message.get("payload") {
            decode_gmail_part(payload, &mut bodies, &mut attachments);
        }
...
async fn calendar_content(
    ...
    let value = send_json("google-calendar", tokens, Method::GET, url, None).await?;
...
pub(crate) async fn execute_action(
    ...
    let resource_id = match action.connector_id.as_str() {
        "google-drive" => execute_drive_action(&tokens, action).await?,
        "gmail" => execute_gmail_action(&tokens, action).await?,
        "google-calendar" => execute_calendar_action(&tokens, action).await?,
        _ => {
            return Err(error(
                &action.connector_id,
                "configuration-required",
...
async fn execute_drive_action(
    ...
    if action.action == "google-drive.update-file" {
        // Media uploads use a raw body; keep this special case out of the JSON helper.
        let content = required("google-drive", payload, "content")?;
        let client = reqwest::Client::builder()
            .timeout(REQUEST_TIMEOUT)
            .build()
        ...
        if !response.status().is_success() {
            return Err(provider_error("google-drive", response.status(), None));
        }
        let value: Value = response.json().await.map_err(|_| {
            error(
                "google-drive",
                "provider-unavailable",
                "Drive upload response was malformed.",
                false,
            )
        })?;
        return Ok(string(&value, "id").or_else(|| payload.get("fileId").cloned()));
    }
    let value = send_json("google-drive", tokens, method, url, body).await?;
...
fn build_mime(payload: &BTreeMap<String, String>) -> Result<String, ConnectorCommandError> {
    ...
    Ok(format!(
        "{}\r\n\r\n{}",
        headers.join("\r\n"),
        payload.get("body").cloned().unwrap_or_default()
    ))
}
...
async fn execute_gmail_action(
    ...
    let value = send_json("gmail", tokens, Method::POST, url, Some(body)).await?;
...
pub(crate) async fn execute_read_tool(
    ...
    let value = match (connector_id, operation) {
        ...
        ("google-drive", "metadata") => {
            ...
            send_json(connector_id, &tokens, Method::GET, url, None).await?
        }
        ("gmail", "search") => {
            let query = object.get("query").and_then(Value::as_str).unwrap_or("");
        ...
        ("google-calendar", "calendars") => {
            ...
            let url = api_url(CALENDAR_API, "users/me/calendarList")?;
            send_json(connector_id, &tokens, Method::GET, url, None).await?
        }
