...
fn load_store_from_sqlite(instance_id: &str) -> Result<Option<SchedulerStore>, String> {
    let result = crate::store::with_store(|store| {
        store.with_conn(|conn| {
            let jobs = crate::store::repos::scheduled_job::list(conn, store, "")?;
            let queue = crate::store::repos::scheduler_queue::list(conn, store, "")?;
            Ok((jobs, queue))
        })
    })?;
...
pub fn list_scheduler_jobs(app: AppHandle) -> Result<Vec<ScheduledJob>, String> {
    // Prefer the production SQLite authority; fall back to the legacy JSON file
    // in the unit-test path (no global store).
    if let Some(store) = load_store_from_sqlite("unset")? {
