...
- Grok Bot-style teammate workspace with durable profiles, learned responsibilities, multi-agent missions, and a live-work rail.
- Config-gated hosted teammate computer foundation: Cloudflare Sandbox terminal/filesystem, approval-bound recurring natural-language agent routines and exact-program alarms, Browser Run observation plus click/fill/select/key/viewport-scroll/bounded-history controls and temporary human takeover, with Convex-provisioned per-workspace/teammate authority.
...
| **Teammates & Team Missions** | **Implemented locally; provider-gated** | Named teammate conversations, durable profile/learned-responsibility state, focused and parallel mission flows, live progress, and approval return. Real model work still requires an installed/authenticated provider or API credential. |
| **Hosted Teammate Computer** | **Implemented foundation; deployment-gated** | Cloudflare Sandbox/Container/DO terminal and persistent `/workspace`; Workers AI-backed recurring natural-language routines with explicit standing read/write/process authority, 1–8 tool-step bounds, pause/resume/cancel, results, and 20-run evidence history; exact-program alarms with 20 durable occurrence records and explicit transient output inspection; and Cloudflare Browser Run navigation, observations, stale-safe approval-bound click/fill/select/key/viewport-scroll/download controls, bounded atomic downloads into `/workspace/downloads`, 32-entry public-HTTPS Back/Forward history, JPEG preview, and short-lived Live View takeover. These continue while Fable is closed. Provisioning and scoped execution authority flow through Convex and the native credential boundary. It is not deployed or live-credential validated in this checkout. |
...
| **Legacy Browser Preview Mode** | **Preview/fixture-only** | The older generic browser-automation preview remains synthetic. Hosted teammates instead use the separate deployment-gated Browser Run path described above. |
