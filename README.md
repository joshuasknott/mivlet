...
- One provider-driver registry with stable instance ids for ChatGPT/Codex,
  ...
  Mivlet's one-time approval boundary. Direct OpenAI, Anthropic, xAI, OpenRouter
  and custom API turns use the bundled OpenCode V2 embedded host, with native
  credential custody and Mivlet tool approvals. OpenRouter routes through its
  OpenAI-compatible API with the exact selected model id; Mivlet adds no
  fallback, and OpenRouter performs its own documented downstream provider
  routing. User-image turns retain the audited visual
