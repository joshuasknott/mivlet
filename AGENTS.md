...
Independent, provider-neutral, local-first AI agent workspace. Preserve the quiet conversation with named agents and keep account identity, workspace data, and provider credentials separate. Signed-out users authenticate; signed-in users connect a usable AI provider before entering their local workspace. Returning users with a usable provider open directly. Keep hosted and planned features distinct from implemented local behavior.
