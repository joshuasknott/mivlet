...
Fable's supported Google model route is Google Antigravity over the Agent
...
Fable downloads the official Windows x64 Antigravity ACP release into app data.
...
Antigravity owns Google OAuth state inside a per-Fable-account profile. Fable
...
explicit Fable sign-in action validates and opens the exact Google OAuth URL
...
model, prompt, stream updates, and cancel or finish. Fable initially advertises
...
to perform an action; it is not a Fable tool call. Fable therefore sends it
through the same visible approval gate but does not execute the action itself.
After a one-time grant or denial, Fable selects the corresponding ACP option.
Missing options, missing approval wiring, stale requests, and cancellation all
fail closed. This avoids both bypassing Fable's approval UI and executing the
same effect twice.
