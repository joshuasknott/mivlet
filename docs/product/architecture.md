...
Implemented runtime commands cover approval resolution, one-time execution permits, approval audit persistence, native agent-run journaling and restart recovery, local text-file import, imported knowledge persistence, memory control state, approval-gated memory promotion, memory export formatting, runtime snapshot recovery, owner-filtered portable workspace copies, exact-owner Project copies, deliberate Project Connection selection, and lexical cited retrieval over workspace sources. A Project stores at most 32 canonical opaque Connection IDs in its encrypted content. New selections must resolve to an authorized exact-owner user Connection or explicitly workspace-shared Connection; retaining a now-unavailable prior selection creates no authority. Connector context is allowlisted before chunking and ranking, while the native capability registry independently reloads the Project and requires the resolved native or MCP Connection ID before any Project-scoped grant is prepared or consumed. Selection therefore narrows existing workspace authority and never creates a grant, credential, billing, provider, placement, or privacy transition. Portable private-document traversal filters by the authenticated owner before decryption; Project copies additionally require the exact active member-owned private Project and exact-owner conversations, and carry no credential, workspace-setting, machine-state, general-audit, unrelated-record, or external-handoff authority. Their portable records retain Project creator/member ownership and conversation/message member ownership. Import accepts that authority only for the same active private member and original internal creator, rejects substitution or removal before any partial commit, preserves conversation titles, and cannot act as an ownership-transfer path. Ownerless legacy archives remain ownerless. Development and test builds may use explicitly labelled preview adapters; production never silently substitutes fixtures for an unavailable native runtime.
