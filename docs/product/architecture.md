...
- Each run is journaled in `agent-runs.json` without credentials. Streaming, retry, approval-wait, completion, cancellation, failure, and interrupted-restart states are recoverable.
...
Provider-specific live read/write adapters are intentionally not implemented yet. The shared extension points, native auth/token lifecycle, fixtures, and fail-closed command surface are implemented so those adapters can be developed independently. Provider console registrations, broker deployment, and provider verification remain deployment configuration.
...
- Recovered native runs are marked interrupted, retain partial transcript/usage/pending approval identifiers, and require an explicit retry or resume path.
