...
- GitHub, Notion, Slack, Vercel, Linear, Google Drive, Gmail, and Google Calendar route through the configured HTTPS auth broker. The broker is limited to authorization, handoff, token refresh, identity, and revocation; it is not a general connector proxy.
