...
- [ ] Add explicit authorization including OAuth/PKCE where appropriate.
  - Repo-local progress: after a remote discovery attempt receives `401`, the native boundary parses a bounded `WWW-Authenticate: Bearer` challenge, accepts only a quoted credential-free public-HTTPS `resource_metadata` URL, normalizes and bounds the challenged scopes, stores neither header nor secret, and gives challenged scopes precedence for that exact account/workspace/configuration. When no valid challenge URL is present, discovery tries the MCP 2025-11-25 fallback URI order. Every metadata fetch uses the same HTTPS-only public-DNS-pinned and no-redirect egress policy, bounds JSON to 256 KiB, requires an exact canonical resource and issuer, validates authorization/token/registration endpoints, bounds scopes, and refuses authorization servers that do not advertise S256 PKCE and authorization-code flow. Client registration now follows an explicit fail-closed preference order: exact issuer-specific pre-registration, a configured public HTTPS Client ID Metadata Document when advertised, Dynamic Client Registration when available, then manual advanced client information. Fable remains a public S256 PKCE client and never embeds or returns a client secret. Pre-registration configuration is bounded and exact-issuer keyed; malformed configuration cannot silently fall through to another strategy. The accepted rationale and external metadata-document gate are recorded in the MCP OAuth client-registration ADR. Only a secret-free summary and selected strategy cross to React, which still states explicitly that account connection is not available yet. Multi-challenge compatibility beyond a leading Bearer challenge, loopback callback, state/verifier lifecycle, registration execution, token exchange/refresh/revocation, OS credential custody, authenticated MCP requests, step-up, and live-provider evidence remain open. The discovery order follows the [MCP 2025-11-25 authorization contract](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization).
- [ ] Add trust classification, discovery, and per-tool/resource enablement.
  - Repo-local progress: local STDIO and remote Streamable HTTP Connections are classified `user-managed`. Their bounded encrypted discovery inventory is supporting metadata only, recorded against the exact live native session and canonical Connection revision. Every tool and resource starts disabled; Settings exposes an explicit subset selection, and native persistence rejects stale revisions or names/URIs absent from current discovery. An advanced user can now explicitly bind exactly one discovered, enabled tool to `knowledge.content.search`. Fable, not the server, fixes that binding to contract `fable.connected-source-search.v1`, consequence `read`, and trust `untrusted`; unsupported capabilities, duplicate bindings, disabled tools, and stale revisions fail closed. The binding remains inside the encrypted Connection payload and safe projections contain no credentials. Rediscovery automatically removes both enablement and semantic bindings for tools the server no longer advertises, while native tool/resource `list_changed` handling immediately revokes the live session's discovery freshness. Strict result-contract validation and ordinary agent-run routing through the binding remain the next substitution slice. Additional trust classes and live cross-transport policy evidence remain open.
- [ ] Apply Fable capability grants, approvals, budgets, audit, and prompt-injection defenses.
  - Repo-local progress: local STDIO and remote Streamable HTTP tool execution share an exact native approval and audit boundary. Only a currently discovered and explicitly enabled tool on the live Connection revision can be proposed; the transport is bound into the proposal fingerprint; approval metadata contains the proposal fingerprint plus at most 16 safe argument field paths and at most 16 HTTP(S) destination origins, never argument values, URL paths, query strings, bodies, or credentials; only a fresh `once` decision can mint a short-lived one-use session permit; Rust alone constructs the call; and response audits record status, tool, Connection, actor, and normalized error without result content. Remote responses must correlate to the exact native request before success; missing, rejected, closed, or malformed transport outcomes consume the pending audit as failure. Before any result can reach product code, the portable client and both exact desktop adapters normalize it into a bounded Fable-owned contract with `trust: untrusted` and `instructionAuthority: none`; text and embedded text are truncated, links are metadata only, binary image/audio data is discarded, structured content remains a bounded marked JSON string, and unknown/oversized shapes fail closed. Credential-shaped arguments, embedded URL credentials, renderer tool-call frames, prompts, sampling, and server-request responses also fail closed. Semantic binding is now explicit and revision-fenced, but capability-grant enforcement and strict cited-search result validation are not yet wired into MCP execution. Mission integration, richer destination/data-routing policy, authenticated-remote execution, and live prompt-injection/parity evidence remain open. Result validation follows the [MCP 2025-11-25 tool-result security guidance](https://modelcontextprotocol.io/specification/2025-11-25/server/tools).
