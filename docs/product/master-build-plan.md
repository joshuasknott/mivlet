...
- [ ] Complete invitation, acceptance, role, and removal lifecycle.
  - Repo-local evidence: Convex now implements authenticated direct-inbox invitations for existing internal users, exact-recipient acceptance, pending-invite listing/revocation/expiry, role changes, suspension/restoration, terminal removal, last-owner protection, device revocation, idempotency, and hashed session audit attribution. Authenticated bootstrap also caches only bounded display claims, masks verified email before storage, clears withdrawn hints, and never uses profile data for identity or authorization. A bounded roster query joins only active internal accounts with active or paused memberships, omits terminal removal history, and projects the exact role and lifecycle controls permitted for each target. Self-management and no-op role changes are rejected, admins cannot manage owners, and accepted pause/removal revokes the target's workspace device links. Native replaces hosted member IDs with bounded process-local action references tied to the exact account generation, active workspace, current member, target revision, and projected controls; it rechecks those facts before and after the fixed hosted mutation and returns only a secret-free outcome. Workspace Settings resets immediately with account and workspace context, supports guarded invitation acceptance, shows a calm `People with access` list, uses explicit role saving and projected pause/restore controls, and requires focused confirmation for permanent removal. Registered-handler, native, runtime, and component tests exercise authorization-before-write, device revocation, exact replay, stale revisions, response loss, account/workspace switching, Strict Mode completion, and same-tick duplicate actions. A deployed two-account journey plus safe verified-attribute invitation creation remain open.
