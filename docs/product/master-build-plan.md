...
- [ ] Add explicit authorization including OAuth/PKCE where appropriate.
  - Repo-local progress: after a remote discovery attempt receives `401`, Settings can ask the native boundary to verify the server's standard well-known protected-resource and authorization-server metadata. Discovery tries the MCP 2025-11-25 fallback URI order, performs every metadata fetch through the same HTTPS-only public-DNS-pinned and no-redirect egress policy, bounds JSON to 256 KiB, requires an exact canonical resource and issuer, validates authorization/token/registration endpoints, bounds scopes, and refuses authorization servers that do not advertise S256 PKCE. Only a secret-free summary crosses to React, which states explicitly that account connection is not available yet. `WWW-Authenticate` challenge parsing, challenged-scope precedence, client registration selection, loopback callback, state/verifier lifecycle, token exchange/refresh/revocation, OS credential custody, authenticated MCP requests, step-up, and live-provider evidence remain open. The discovery order follows the [MCP 2025-11-25 authorization contract](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization).
