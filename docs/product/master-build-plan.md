...
- [ ] Complete keyboard, screen-reader, contrast, focus, motion, caption, and voice accessibility.
  - Repo-local progress: Settings and Scheduled Task creation now share a nested-dialog-aware keyboard focus scope. Opening either modal moves focus to its first useful control, Tab and Shift+Tab wrap within the top-most dialog, Escape closes it, and focus returns to the exact opener. Component and full-shell tests pin those behaviors. A current browser-preview pass also verified the Privacy layout at 1,280 px, 640 px, and 420 px without horizontal overflow or clipped interactive controls. Other dialogs, packaged-WebView assistive-technology behavior, full contrast/motion/caption coverage, and native voice accessibility remain open, so the full item stays unchecked.
