...
- [ ] Add optional project instructions, goals, knowledge, connections, missions, routines, artifacts, and activity.
  - Repo-local evidence: the Project detail page now includes a bounded Activity summary for exact-project Mission runs, canonical Routines, current Artifacts, and the workspace-scoped Connections referenced by those Routines or Artifact evidence. Active and archived Project Mission history is read only through exact durable conversation IDs already filtered to the current authenticated private member. Archived conversations and Mission summaries are deliberately non-interactive, so inspection does not reopen a composer or mutation path. Every listed Mission exposes its bounded durable steps, progress, usage, budget, acceptance state, and next action; an active Project can record an identified-human accept or needs-revision decision with the exact encrypted run revision and event-head fence, then finalize only after every required human check is settled. Active Project Mission work can also request a cooperative stop: the hook reloads the exact encrypted Run, uses its current revision and event-head sequence, and appends the existing authenticated cancellation fact with a fresh identity. The UI says that Fable will accept no further result and that already-sent work cannot be undone. The active graph aborts its provider boundary, waits for every in-flight worker to settle, then calls the native cancellation finalizer against the newly reloaded exact head; restart recovery owns terminalization when no active graph remains. A partial, failed, or cancelled general Mission in an active Project can start a fresh run only after Fable reloads the exact current Project thread twice around the message read, verifies its unchanged revision and Project membership, and finds one canonical `/mission` user record plus its assistant record under the exact old run ID. The fresh path re-enters current provider and scope selection; it does not revise or reuse the old journal, output, route, grants, or approvals. Connection names and health come only from the current authenticated native projection, while missing or revoked references show `Needs attention`. Project Routine pause, resume, and delete controls carry the exact Project ID and current encrypted Routine revision into the existing native transition boundary, then refresh the scoped list; archived Projects remain read-only. Project Artifacts open the same exact-owner detail contract as Knowledge, including immutable version selection, review state, citations, exact-version Markdown/JSON export, and explicitly confirmed same-owner private-Project handoff without authority transfer. An active Project can deliberately select up to 32 existing Connections from its exact private owner plus explicitly workspace-shared Connections. The encrypted Project stores only canonical opaque IDs; selection grants no capability, copies no credential or provider authority, and a new selection must still be authorized. Missing/revoked prior selections stay visible for attention or removal. The Project Knowledge surface can search an already-selected first-wave Connection and import an exact result into that Project's encrypted private Knowledge. Native code rejects an omitted, substituted, unselected, or changed Connection before provider egress, then rechecks the encrypted Project revision and selection after egress before persistence. Project runs exclude unselected connector sources before chunking or ranking, and the native capability registry independently requires the resolved native or MCP Connection to match the Project's saved selection before preparing or consuming a grant. Standalone workspace reads remain unchanged. The same page can atomically create a new plaintext JSON copy of one exact active member-owned private Project. Native export re-derives the active workspace/member, rejects foreign, shared, deleted, or ambiguous legacy Projects, includes only exact-owner Project conversations and owner/project-qualified private documents plus the supported project-scoped portable sections, and excludes credentials, workspace/account settings, machine state, audit history, and external Artifact handoff authority. The portable importer preserves Project, conversation, and message owner evidence and conversation titles only when the archive names the exact active private member and original internal creator; missing, substituted, cross-owner, or structurally incomplete current authority fails closed, existing rows remain untouched, and no ownership transfer is inferred. Ownerless legacy archives retain their prior compatibility path without being upgraded to private authority. Identified-human acceptance, durable stop requests with active-graph finalization, and safe fresh rerun are implemented.
...
- [ ] Complete source ingestion, provenance, trust, freshness, and lifecycle.
  - Repo-local evidence: supported local text files now have authenticated workspace/project import, provenance, trust, search, disable/re-enable, tombstone delete, and explicit one-shot content refresh. First-wave Connector search results are stamped natively with the exact authenticated Fable Connection and selection revision; import rejects missing, changed, or cross-Connection evidence, persists the untrusted source in an encrypted owner/workspace-scoped document, redacts secret-shaped provider content, reloads after restart, and durably supports disable, re-enable, tombstone delete, and explicit re-import refresh. Project-scoped search/import uses the exact active private Project and one of its saved Connections, revalidates both after provider egress, and persists the normalized source under the owner/project-qualified encrypted document. Background synchronization and live-provider validation remain open.
- [ ] Filter retrieval by member, workspace, project, department, and Connection before ranking.
  - Repo-local evidence: member, workspace, project, and exact Fable Connection authority now filter Knowledge and Memory before ranking, including pins, explicit source requests, citations, and connector-derived memory. Missing Connection provenance fails closed; revoked, unavailable, unhealthy, or offline exact Connections cannot enter corpus statistics, lexical/semantic scoring, context assembly, or receipts. Department filtering remains closed until Departments genuinely exist.
...
- [ ] Implement local STDIO MCP.
  ...
  - Repository fixture evidence: the owned STDIO server now exercises the native child-process builder through initialization, exact tool and resource discovery, a contract-versioned cited search, cancellation of a long-running call, `tools/list_changed` invalidation, clean shutdown, unexpected exit, and fresh-process restart. Its result deliberately contains instruction-shaped citation text and crosses the production native normalizer into the same `external-untrusted`, no-instruction-authority, Fable-owned citation contract used by the cited Mission. Malformed/oversized frame and result cases remain covered at the adjacent native decoder and normalizer boundaries. This closes the repository-local process-to-semantic evidence gap without claiming the authenticated Tauri UI journey.
...
- [ ] Apply Fable capability grants, approvals, budgets, audit, and prompt-injection defenses.
  ...
  - Mission update: the repository-local cited-brief path now consumes an explicitly bound MCP implementation under the exact scoped capability grant and per-search approval, persists only the native-attested normalized tool receipt, and applies the same citation policy before provider egress. The owned STDIO fixture now supplies native process-to-semantic and prompt-shaped-content evidence for that contract. This supersedes the earlier sentence that listed repository Mission integration as open; richer routing policy, authenticated packaged-app evidence, and live third-party parity remain open.
...
authority substitution, and partial deterministic aggregation. The fixed
two-producer/reviewer journey remains available for its specialized comparison
artifact, while the normal composer now also accepts the bounded independent
worker shape described below.
...
- [ ] Complete backup and restore for data, artifacts, configuration, and required keys.
  - Repo-local progress: the desktop can create a consistent non-overwriting encrypted SQLite backup containing local data, artifact records, and database-backed configuration. A Fable-owned encrypted marker records the backup format, schema, creation time, matching-key requirement, and credential exclusion; creation immediately reopens the candidate read-only, runs SQLite integrity validation, and proves the current OS-held vault key can decrypt the marker. Privacy settings state the boundary plainly. Exact-confirmation restore rejects linked, corrupt, unmarked, newer-schema, active-database, and wrong-key files, copies a verified candidate to a fixed pending slot without replacing live data, and requires restart. Startup validates it again, checkpoints the current WAL, preserves the prior database under a unique recovery filename, activates the candidate, and restores the prior database if open or migration fails. Native tests cover matching/wrong keys, marker tamper, restart activation, retained prior data, and unchanged-current failure. Provider/OAuth credentials and the vault key remain separate OS credential-store lifecycles and are not exported; portable workspace export remains the cross-device/key-loss route. Packaged restart observation and a deliberate key export or escrow decision remain open, so the full item stays unchecked.
- [ ] Complete migration rollback/recovery and crash recovery.
- [ ] Complete account/workspace/data/artifact export and deletion.
  - Repo-local progress: portable workspace export/import, an exact-owner plaintext Project-copy export, exact artifact export/deletion lifecycles, project deletion/detachment, and exact-confirmation deletion of this installation's local database and compatibility files are product-composed. Local deletion returns a restart-required native receipt and explicitly does not claim to delete hosted account/workspace data, revoke provider-side keys, or remove OS-held provider credentials. Hosted account/shared-workspace deletion and deployed multi-account evidence remain open, so the full item stays unchecked.
  - Repo-local progress: Privacy settings can save and import the active authenticated workspace as a portable plaintext JSON copy through the existing versioned manifest. Native code derives the workspace instead of trusting the renderer, filters owner-qualified private documents to the exact active owner before decrypting them, validates referential integrity and secret absence, includes the manifest's supported projects, conversations, artifacts, canonical Routine definitions with complete immutable versions/triggers/occurrence history, legacy schedules/workflows, and other user-owned sections, reports the explicit omitted credential/cache/machine state, writes through a temporary file, syncs it, and atomically persists only to a new `.json` target. Existing files are never overwritten and linked destination folders fail closed. Import requires the exact `import workspace copy` confirmation, accepts only a bounded regular link-free `.json` file, keeps content entirely outside renderer state, rejects invalid/newer/credential-shaped manifests, and applies inside one rollback-safe transaction with skip-existing conflict handling. Imported Connections, schedules, scheduled jobs, and active Routines remain disabled or paused; Routine ownership must match the exact active account/workspace/member. The Project detail can also save one exact member-owned private Project as a readable portable-shaped copy while excluding unrelated records and non-source handoff authority. That copy can be imported into an empty matching workspace/member: Project, conversation, and message ownership plus conversation titles are preserved, exact original creator/member evidence is required, and substituted or stripped current authority is rejected without importing partial rows. It cannot transfer a Project to another member. Ownerless legacy archives remain compatible but gain no private authority. Scheduler writer authority, leases, cursors, retries, and legacy-migration rollback evidence remain node-local and are never imported. Browser preview reports native export operations unavailable rather than manufacturing evidence. The separate encrypted recovery backup remains the same-device/matching-key path. Unified hosted-account deletion, deployed shared-workspace deletion evidence, and packaged recovery observation remain open, so the full item stays unchecked.
...
- [ ] Add cost visibility, budgets, cancellation, kill switches, and incident audit.
  - Repo-local progress: Mission and provider boundaries already retain exact route-bound token usage, known-model cost provenance, conservative pre-egress ceilings, duration/tool/attempt/worker budgets, durable budget failures, and cooperative cancellation. Privacy settings now add an encrypted workspace-wide execution pause with exact typed pause/resume confirmation and optimistic resume fencing. It blocks new native-provider, local-model, ACP, Codex app-server, approved MCP-tool, legacy scheduler, and canonical Routine execution/leases; corrupt control state fails closed. Each transition records a static secret-safe action-history fact. The interface states that already accepted external effects cannot be undone and that owning-runtime cancellation remains separate. Broader exact pricing, cancellation of every local subprocess already in flight, deployed incident telemetry, and provider-side effect reconciliation remain open, so the full item stays unchecked.

### Wave 10C - Product quality

- [ ] Complete keyboard, screen-reader, contrast, focus, motion, caption, and voice accessibility.
  - Repo-local progress: every current `aria-modal` entry point now uses one explicit nested-dialog-aware keyboard focus boundary. It covers account Settings, workspace settings, provider setup, Scheduled Task creation, connector details, chat history, device removal, member removal, and MCP configuration confirmation. Opening moves focus to the first useful control, Tab and Shift+Tab wrap within the top-most dialog, Escape closes it where cancellation is safe, and focus returns to the opener or an explicit stable invoking control when a transient menu has unmounted. The same scope makes every background DOM branch inert for keyboard and assistive-technology navigation while the dialog is active, using reference-counted claims so a nested confirmation cannot re-expose the parent background when it closes. Component and full-shell tests pin representative top-level and nested-confirmation behavior. A final-cascade reduced-motion policy now removes smooth scrolling, reduces every transition to an effectively instant change, and limits every animation—including indefinite spinners and run pulses—to one effectively instant iteration when the operating system requests reduced motion. Shared secondary-text and positive/caution/destructive semantic tokens now meet at least WCAG AA 4.5:1 against their supported light/dark surface and status backgrounds; source-level regression tests pin both policies. A current browser-preview pass also verified the Privacy layout at 1,280 px, 640 px, and 420 px without horizontal overflow or clipped interactive controls. Packaged-WebView assistive-technology behavior, component-specific contrast/caption coverage, and native voice accessibility remain open, so the full item stays unchecked.
...
- [ ] Remove fixtures, misleading states, duplicate paths, stale copy, and dead ends.
  - Repo-local progress: the Schedules page now labels its browser-only records as `Preview only` and states that synthetic schedules stay in the browser and cannot run provider work; the native page does not receive that fixture copy. User-facing double-encoded punctuation was removed from Schedule, Mission approval, and MCP account status controls. The placeholder Departments page is no longer offered in primary navigation while that explicitly deferred product area is unavailable; its compatibility route remains so old local state does not require a destructive migration. Broader fixture/dead-end and copy review remains open, so the full item stays unchecked.
- [ ] Meet startup, memory, storage, retrieval, streaming, and long-run performance budgets.
  - Repo-local progress: the standard `pnpm check` and Windows CI gates now enforce the measured desktop JS/CSS aggregate, gzip aggregate, initial-entry payload, CSS payload, and every current lazy product route against narrow explicit byte ceilings. The historical baseline is retained, React Query plus React scheduler bridges share the React vendor chunk without a circular dependency, and the cleanup programme reduced the initial entry to 609,277 B (below 600 KiB), aggregate JavaScript/CSS to 1,411,960 B, and CSS to 195,056 B without raising budgets or deferring required startup work. The same gate runs deterministic connector shaping, Knowledge retrieval/retention, encrypted-store/cache, and scheduler-queue fixture checks; protocol, Mission, Routine, and MCP suites separately enforce bounded frames, outputs, histories, workers, iterations, leases, and retries. Packaged Windows cold start, comparable RSS, real-provider streaming, and long private soak evidence require the packaged application and remain open, so the full item stays unchecked.
