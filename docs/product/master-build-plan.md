...
- [ ] Complete account/workspace/data/artifact export and deletion.
  - Repo-local progress: Privacy settings can now save the active authenticated workspace as a portable plaintext JSON copy through the existing versioned manifest. Native code derives the workspace instead of trusting the renderer, validates referential integrity and secret absence, includes projects, conversations, artifacts, Routines and other supported user-owned sections, reports the explicit omitted credential/cache/machine state, writes through a temporary file, syncs it, and atomically persists only to a new `.json` target. Existing files are never overwritten, linked destination folders fail closed, and browser preview reports the operation unavailable rather than manufacturing an archive. The separate encrypted recovery backup remains the same-device/matching-key path. Portable import UI, unified hosted-account deletion, and deployed shared-workspace deletion evidence remain open, so the full item stays unchecked.
