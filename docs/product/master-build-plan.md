...
- [ ] Implement Connection authorization, health, refresh, degradation, revocation, removal, and recovery.
  - Repo-local evidence: OAuth completion now treats secure-store persistence as a reversible commit until compatibility metadata is durable. It captures the exact previous credential value, fails closed if that value cannot be read, restores it when reauthorization metadata fails, removes a newly created credential on the same failure, and returns only secret-free recovery errors. Canonical Connection participation in that coordinated commit, durable health transitions, and complete revoke/remove/recovery journeys remain open.
