...
- [ ] Complete source ingestion, provenance, trust, freshness, and lifecycle.
  - Repo-local evidence: supported local text files now have authenticated workspace/project import, provenance, trust, search, disable/re-enable, tombstone delete, and explicit one-shot content refresh. First-wave Connector search results are stamped natively with the exact authenticated Fable Connection and selection revision; import rejects missing, changed, or cross-Connection evidence, persists the untrusted source in an encrypted owner/workspace-scoped document, redacts secret-shaped provider content, reloads after restart, and durably supports disable, re-enable, tombstone delete, and explicit re-import refresh. Project-scoped Connector ingestion, background synchronization, and live-provider validation remain open.
