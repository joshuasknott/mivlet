...
- [ ] Complete invitation, acceptance, role, and removal lifecycle.
  - Repo-local evidence: Convex now implements authenticated direct-inbox invitations for existing internal users, exact-recipient acceptance, pending-invite listing/revocation/expiry, role changes, suspension/restoration, terminal removal, last-owner protection, device revocation, idempotency, and hashed session audit attribution. Authenticated bootstrap also caches only bounded display claims, masks verified email before storage, clears withdrawn hints, and never uses profile data for identity or authorization. A bounded roster query joins only active internal accounts with active or paused memberships and omits terminal removal history. The native inbox and roster are fixed-path and account-bound across every hosted await; roster parsing rejects cross-workspace, duplicate, oversized, private-metadata, malformed-profile, and current-member mismatches. Workspace Settings identifies the server-owned workspace, resets immediately with account and workspace context, supports guarded focus-safe invitation acceptance, and shows a read-only `People with access` list without preview fallback or rendered member IDs. Registered-handler, native, runtime, and component tests exercise authorization-before-write, response-loss replay, account switching, stale and Strict Mode UI completions, and same-tick duplicate actions. A deployed two-account journey plus safe invitation creation and member mutation controls remain open.
