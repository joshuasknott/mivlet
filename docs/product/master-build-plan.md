...
- [ ] Add explicit authorization including OAuth/PKCE where appropriate.
  - Repo-local progress: after a remote discovery attempt receives `401`, the native boundary parses a bounded `WWW-Authenticate: Bearer` challenge, accepts only a quoted credential-free public-HTTPS `resource_metadata` URL, normalizes and bounds the challenged scopes, stores neither header nor secret, and gives challenged scopes precedence for that exact account/workspace/configuration. When no valid challenge URL is present, discovery tries the MCP 2025-11-25 fallback URI order. Every metadata fetch uses the same HTTPS-only public-DNS-pinned and no-redirect egress policy, bounds JSON to 256 KiB, requires an exact canonical resource and issuer, validates authorization/token/registration endpoints, bounds scopes, and refuses authorization servers that do not advertise S256 PKCE and authorization-code flow. Only a secret-free summary crosses to React, which states explicitly that account connection is not available yet. Multi-challenge compatibility beyond a leading Bearer challenge, client registration selection, loopback callback, state/verifier lifecycle, token exchange/refresh/revocation, OS credential custody, authenticated MCP requests, step-up, and live-provider evidence remain open. The discovery order follows the [MCP 2025-11-25 authorization contract](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization).
