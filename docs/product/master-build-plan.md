...
- [ ] Complete invitation, acceptance, role, and removal lifecycle.
  - Repo-local evidence: Convex now implements authenticated direct-inbox invitations for existing internal users, exact-recipient acceptance, pending-invite listing/revocation/expiry, role changes, suspension/restoration, terminal removal, last-owner protection, device revocation, idempotency, and hashed session audit attribution. The native inbox is fixed-path and account-bound across every await and the final local commit, uses stable per-account/invitation replay keys, strips control-plane evidence before IPC, reconciles accepted workspaces across restart, and preserves accepted truth when refresh is unavailable. Workspace Settings identifies the server-owned workspace, resets immediately with account context, and supports a guarded, focus-safe acceptance action without preview fallback. Unavailable invitation workspaces are omitted without hiding other valid invitations. Registered-handler, native, runtime, and component tests exercise authorization-before-write, response-loss replay, account switching at the commit boundary, stale UI completions, and same-tick duplicate actions. A deployed two-account journey plus safe invitation creation and member-administration UI remain open.
