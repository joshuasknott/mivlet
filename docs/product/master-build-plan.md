...
- [ ] Implement the semantic capability registry and resolver.
  - Repo-local evidence: the first provider-neutral execution slice is wired through the real model-tool boundary. The advertised `connection-read` tool requests `source.repository.list` without naming GitHub; the native registry resolves only its declared GitHub `repositories.list` implementation, requires the selected canonical Connection, exact `repo` scope, current account/workspace authority, and usable health evidence, then executes the existing live adapter through the unchanged read-only approval/audit boundary. The adapter token is bound to the exact opaque Connection resolved before egress, so a same-account selection change conflicts instead of silently reading through another provider account. Unknown capabilities, absent Connections, missing scopes, unhealthy/offline state, changed identity, changed selection, and unavailable encrypted state fail closed with the canonical resolution vocabulary. The result identifies the opaque Connection and `adapter-validated` evidence without exposing provider account ids or credentials. Expanding the registry beyond this proved read capability, adding alternative implementations/provider routes, and routing missions/routines through it remain open.
