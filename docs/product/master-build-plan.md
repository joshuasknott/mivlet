...
- [ ] Complete keyboard, screen-reader, contrast, focus, motion, caption, and voice accessibility.
  - Repo-local progress: every current `aria-modal` entry point now has an explicit keyboard focus boundary. The shared nested-dialog-aware scope covers account Settings, workspace settings, Scheduled Task creation, connector details, chat history, device removal, member removal, and MCP configuration confirmation; the provider catalogue retains its equivalent dedicated trap. Opening moves focus to the first useful control, Tab and Shift+Tab wrap within the top-most dialog, Escape closes it where cancellation is safe, and focus returns to the opener or an explicit stable invoking control when a transient menu has unmounted. Component and full-shell tests pin representative top-level and nested-confirmation behavior. A final-cascade reduced-motion policy now removes smooth scrolling, reduces every transition to an effectively instant change, and limits every animation—including indefinite spinners and run pulses—to one effectively instant iteration when the operating system requests reduced motion; a source-level regression test pins the global coverage. A current browser-preview pass also verified the Privacy layout at 1,280 px, 640 px, and 420 px without horizontal overflow or clipped interactive controls. Packaged-WebView assistive-technology behavior, full contrast/caption coverage, and native voice accessibility remain open, so the full item stays unchecked.
