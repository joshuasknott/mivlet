...
- Google connectors: Drive, Gmail, and Calendar expose authenticated reads and approval-gated writes, active-token granted-scope checks, refresh-token preservation without historical scope merging, explicit active-Connection selection, bounded responses, cancellation, and normalized provider errors. The multi-account compatibility boundary derives an opaque workspace-and-connector-bound Fable Connection ID for each account and substitutes it in connector manifests, authorization results, and account options before they cross into the renderer; raw provider account IDs neither authorize selection nor leave the native boundary. It projects canonical lifecycle, authorization, health, credential-custody, and credential-state vocabulary. Safe Connection projections reload for connected manifests after desktop hydration and are cleared before a workspace switch can expose stale results. SQLite schema v19 defines constrained canonical Connection records and quarantines only an opaque proposed identity for each legacy connector-account row; it deliberately does not adopt those rows without an authenticated creator or move credentials. The canonical repository can create and revision-update encrypted native-connector records only under a transaction-current account/workspace/member scope, with stale scope/revision, project scope, invalid state, and cross-workspace secure-store binding rejection; its safe projection excludes secure-store references. Revision-checked canonical lifecycle transitions are also available and reject stale or changed authority. Native account listing reconciles the exact compatibility snapshot into canonical records under one account-generation guard and one SQLite transaction, without revision churn on repeat hydration and without committing after an account switch. It then returns lifecycle, authorization, health, custody, and credential state from exact-ID canonical records and fails closed if a mapping is absent. OAuth exchange prepares without writes; its synchronous commit binds the pending flow to the initiating Fable account and active workspace, holds the exact identity generation, then persists secure-store credential, compatibility metadata, and canonical Connection together. A later failure restores the prior credential and compatibility account set, or removes a brand-new credential, with secret-free recovery errors. Disconnect uses the same account/workspace guard and compensates credential, compatibility selection, and canonical lifecycle persistence before attempting best-effort provider revocation; failed local commits restore the prior local state without exposing secrets. Token refresh snapshots and rechecks the exact compatibility record, credential, canonical lifecycle, and canonical revision across provider egress, then compensates rotated credential, compatibility metadata, and canonical authorization state; a late response cannot revive a concurrently changed Connection. Definitive rejection becomes refresh-required/expired/unhealthy in both local projections without deleting the refresh credential; transient provider failures do not rewrite durable state. Explicit live health checks persist their normalized result to the exact canonical Connection only after rechecking the initiating account and active selection, and never rewrite authorization or credential state. Selection changes and normal credential resolution require the exact canonical record to remain authorized with an available credential under the current account/workspace guard; refresh-required state permits only the retained recovery path. Active-choice and secure-store-reference persistence still use the compatibility store; background health policy and full canonical cutover remain open. External use still requires Google Cloud configuration and applicable verification.
...
| --- | --- | --- |
| Local files, approvals, memory controls, knowledge search, runtime snapshots | **Finished** | Live local runtime paths in Tauri. Pinned context, sources, and memory records are persisted in dedicated SQLite tables (schema v19) with owner-qualified composite key isolation. |
