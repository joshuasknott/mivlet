...
| Local optional projects | **Finished for lifecycle, guidance, conversations, Goals, local Knowledge, and Memory** | Encrypted member-private projects support lifecycle and thread placement, a calm detail page, typed local goals, exact-project local-file import/list/search, deliberate source-to-memory promotion, full memory controls/export, and exact run-context consumption through authenticated Tauri commands. Connections and later project context remain open. |
