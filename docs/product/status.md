...
- Account and workspace (**Wave 1A implementation complete; live validation open**): accepted ADRs and `@fable/protocol` contracts govern identity and tenancy. Clerk supplies authentication facts only; Convex owns stable internal users, identity links, workspaces, memberships, invitations, devices, and hosted authorization. Encrypted SQLite schema v17 stores the per-account workspace directory, active selection, device mirror, workspace-scoped sync state, durable conversations, local project lifecycle, typed local goals, owner-qualified private context, immutable artifact versions, and exact-version private reviews. Native workspace commands fail closed without a current authenticated internal user; portable archives are bound to the active authorized workspace and private owner. The native adapter bootstraps an initial workspace idempotently, reconciles and creates workspaces, switches isolated runtime scope, and revokes account devices without exposing bearer tokens or arbitrary hosted calls to React.
...
- Provider and artifact foundation (**Wave 1C creation plus Wave 2C immutable revisions and private review implemented; full UI/IPC restart validation open**): agent providers support verified add, health checks, credential replacement/reconnect, revoked-auth recovery, and confirmed local removal with explicit provider-side revocation guidance. Provider metadata and keyring credentials are account-owned and unavailable after sign-out or account switching; legacy ownerless provider metadata is quarantined. Model selection uses exact provider-qualified routes and fails closed for unavailable or ambiguous choices. A terminal assistant response can become an encrypted, sourced artifact whose creator/member attribution and citations are derived from authenticated durable records. Schema v17 binds artifacts, versions, and reviews to the exact hosted member or local user with owner-bound encryption, quarantines pre-v16 unowned rows, and stores each edit as an immutable version. Append and review actions use atomic expected-revision/current-version checks; earlier content, hashes, evidence, and resolved review records remain unchanged. Private owners can start a clearly labeled private review, request specific changes, create a new draft version, and mark the exact current version accepted. Multi-member reviewers are not implied before Wave 2D. The desktop shows compact status, history, editing, review controls, plain stale-conflict handling, and persistent keyboard focus. Native artifact creation, revision, and review ignore renderer-supplied authority, actor, time, status, and evidence. Run ownership, message linkage, member isolation, version/review history, archive tamper rejection, and reopen behavior are covered by native tests. The existing minimum-journey test composes native repositories directly; an actual Tauri IPC/UI plus restart validation remains open.
...
- Local recovery: runtime snapshot, approval audit, approval rules, imported knowledge, memory state, connected backend ids, artifacts, immutable artifact versions, and private review history are persisted through encrypted SQLite in Tauri; browser preview still uses localStorage. Schedules, workflows, and knowledge structures are persisted inside the encrypted SQLite database under schema v17.
...
| --- | --- | --- |
| Local files, approvals, memory controls, knowledge search, runtime snapshots | **Finished** | Live local runtime paths in Tauri. Pinned context, sources, and memory records are persisted in dedicated SQLite tables (schema v17) with owner-qualified composite key isolation. |
...
| Private artifacts | **Finished for create, edit, history, lineage, source retention, private review/changes/acceptance, search, exact-version export, ownership, restart, and portable integrity** | Exact-owner artifacts have immutable versions and reviews, atomic stale-write protection, producing-response links, inherited canonical evidence, calm inline editing/review controls, Knowledge search/detail/history, whitelisted Markdown/JSON export, and fail-closed archive validation. Multi-member review and explicit handoff remain open. |
