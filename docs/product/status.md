...
- Account and workspace (**Wave 1A implementation complete; live validation open**): accepted ADRs and `@fable/protocol` contracts govern identity and tenancy. Clerk supplies authentication facts only; Convex owns stable internal users, identity links, workspaces, memberships, invitations, devices, and hosted authorization. Encrypted SQLite schema v16 stores the per-account workspace directory, active selection, device mirror, workspace-scoped sync state, durable conversations, local project lifecycle, typed local goals, owner-qualified private context, and immutable artifact versions. Native workspace commands fail closed without a current authenticated internal user; portable archives are bound to the active authorized workspace and private owner. The native adapter bootstraps an initial workspace idempotently, reconciles and creates workspaces, switches isolated runtime scope, and revokes account devices without exposing bearer tokens or arbitrary hosted calls to React.
...
- Provider and artifact foundation (**Wave 1C creation plus Wave 2C immutable revisions implemented; full UI/IPC restart validation open**): agent providers support verified add, health checks, credential replacement/reconnect, revoked-auth recovery, and confirmed local removal with explicit provider-side revocation guidance. Provider metadata and keyring credentials are account-owned and unavailable after sign-out or account switching; legacy ownerless provider metadata is quarantined. Model selection uses exact provider-qualified routes and fails closed for unavailable or ambiguous choices. A terminal assistant response can become an encrypted, sourced artifact whose creator/member attribution and citations are derived from authenticated durable records. Schema v16 binds artifacts and versions to the exact hosted member or local user with owner-bound encryption, quarantines pre-v16 unowned rows, and stores each edit as an immutable version. Append uses an atomic expected-revision/current-version check, keeps earlier content and hashes unchanged, inherits canonical evidence, and records exact `supersedes` lineage. The desktop shows a compact current version, collapsible history, and labeled markdown editor with plain stale-conflict handling. Native artifact creation and revision ignore renderer-supplied authority and evidence. Run ownership, terminal state, message linkage, member isolation, version history, archive tamper rejection, and reopen behavior are covered by native tests. The existing minimum-journey test composes native repositories directly; an actual Tauri IPC/UI plus restart validation remains open.
...
- Local recovery: runtime snapshot, approval audit, approval rules, imported knowledge, memory state, connected backend ids, artifacts, and immutable artifact versions are persisted through encrypted SQLite in Tauri; browser preview still uses localStorage. Schedules, workflows, and knowledge structures are persisted inside the encrypted SQLite database under schema v16.
...
| --- | --- | --- |
| Local files, approvals, memory controls, knowledge search, runtime snapshots | **Finished** | Live local runtime paths in Tauri. Pinned context, sources, and memory records are persisted in dedicated SQLite tables (schema v16) with owner-qualified composite key isolation. |
...
| Private artifact revisions | **Finished for create, edit, history, lineage, source retention, ownership, restart, and portable integrity** | Exact-owner artifacts have immutable versions, atomic stale-write protection, producing-response links, inherited canonical evidence, a calm inline editor/history view, and fail-closed archive validation. Review, artifact search/exact-version export, and explicit handoff remain open. |
...
- Private artifact archives export only the exact active owner's records. Unsigned archives with citations, inputs, decisions, reviews, or publication claims are intentionally non-importable until Fable has a trusted evidence-transfer mechanism; evidence-free exact-owner artifacts can round-trip only when their native source response already exists in the destination.
