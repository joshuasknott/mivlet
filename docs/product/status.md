...
- Cloud/team backend (**Wave 2D repo-local foundation plus invitation acceptance UI implemented; live multi-member journey open**): Convex is the canonical hosted control plane for Fable-owned users, identity links, workspaces, memberships, direct-inbox invitations, roles, devices, and shared-project authorization. Authenticated handlers implement invitation acceptance/revocation/expiry, role and status transitions, terminal removal, last-owner protection, and device revocation. The fixed native invitation inbox validates the server-owned workspace name, binds every hosted await and the local reconciliation transaction to one opaque current-account generation, derives a stable retry key without exposing it, projects only a safe decision through IPC, and reconciles an accepted workspace into the existing encrypted directory. Response loss, account switching at the final commit boundary, restart, refresh failure, unavailable invitation workspaces, same-tick duplicate clicks, and stale UI completion are covered; Workspace Settings resets on account change, shows an honest desktop-only inbox, and exposes no internal-user-ID creation form. Shared-project mutations derive member/device authority, recompute canonical fingerprints, enforce exact idempotent replay and optimistic revisions, and append one immutable change snapshot per accepted workspace revision. The native sync boundary keeps hosted credentials out of React, stores an encrypted offline outbox and shared mirror, rechecks current authority around network calls, applies contiguous deltas atomically, preserves conflict shadows and durable tombstones, and fails closed when legacy hosted history needs backfill. Deployment, realtime desktop consumption, safe invitation creation, member-administration UI, and live two-account/network validation remain open.
...
| Native voice providers, Convex collaboration | **Mixed** | Dictation still depends on the host Web Speech API. Convex now has hosted membership lifecycle and shared-project revision history plus an explicit authenticated native encrypted cache/outbox adapter. Workspace Settings can safely list and accept direct-inbox invitations; invitation creation, member administration, realtime desktop consumption, deployment, and live multi-member validation remain open. |
