...
- Semantic capability resolver (**first live registry slice implemented; alternatives open**): the model-facing `connection-read` tool declares provider-neutral repository-list, deployment-list, and issue-list capabilities. Native resolution maps them to the existing GitHub, Vercel, and Linear read adapters only when the selected canonical Connection is current, provider-matched, authorized, credential-backed, exactly scoped, and healthy enough; it reports available/degraded truth and adapter-validated evidence, preserves the established read-only approval/audit boundary, and binds provider access to the exact opaque Connection selected before egress. Unknown capability, provider mismatch, missing Connection, scope denial, unhealthy state, identity or selection change, and unavailable encrypted state fail closed. Alternative native/MCP implementations, provider routes, persisted discovery evidence, and mission/routine consumption remain planned.
