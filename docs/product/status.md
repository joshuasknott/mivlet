...
- Knowledge and retrieval: local files and recursive folders can be imported (with path-escape guards), structurally chunked (for Markdown, JSON, CSV, YAML), fingerprinted, scoped, searched, disabled, and deleted. Project local sources now have the same disable/re-enable and tombstone-delete lifecycle. The current workspace Refresh control changes freshness metadata only; it does not reread or reindex file content, so canonical file-update refresh remains open. Retrieval uses reciprocal-rank fusion (RRF, k=60) for hybrid lexical/semantic ranking. Sources and memory records are subject to workspace-scoped composite key isolation (SQLite schema v5) with deletion/forget tombstones.
...
| Local optional projects | **Finished for lifecycle, guidance, conversations, Goals, local Knowledge, and Memory** | Encrypted member-private projects support lifecycle and thread placement, a calm detail page, typed local goals, exact-project local-file import/list/search/disable/delete, deliberate source-to-memory promotion, full memory controls/export, and exact run-context consumption through authenticated Tauri commands. Real file-content refresh, Connections, and later project context remain open. |
