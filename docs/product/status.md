...
- Semantic capability resolver (**first live slice implemented; registry expansion open**): the model-facing `connection-read` tool can request `source.repository.list` without choosing a provider. Native resolution maps that semantic capability to the existing GitHub repository-list adapter only when the selected canonical Connection is current, authorized, credential-backed, correctly scoped, and healthy enough; it reports available/degraded truth and adapter-validated evidence, preserves the established read-only approval/audit boundary, and binds provider access to the exact opaque Connection selected before egress. Unknown capability, missing Connection, scope denial, unhealthy state, identity or selection change, and unavailable encrypted state fail closed. Additional capabilities, alternative native/MCP implementations, provider routes, and mission/routine consumption remain planned.
