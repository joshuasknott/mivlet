...
- Semantic capability resolver (**live native registry slices implemented; alternatives open**): the model-facing `connection-read` tool declares provider-neutral repository-list, file-search, explicitly-shared knowledge-content search, email-search, accessible-channel-list, deployment-list, and issue-list capabilities. Native resolution maps them to the existing GitHub, Google Drive, Notion, Gmail, Slack, Vercel, and Linear read adapters only when the selected canonical Connection is current, provider-matched, authorized, credential-backed, exactly scoped, and healthy enough. Capability-shaped, Google search-shaped, and collaboration search-shaped adapters keep their distinct provider result contracts while sharing available/degraded truth and adapter-validated evidence. Gmail returns normalized message metadata with bounded partial-failure placeholders and does not gain compose/send authority. Notion search is limited to pages and databases explicitly shared with the integration and clamps pagination to the provider limit. Slack global message search is not advertised because the current bot-token broker deliberately lacks the separately governed user-token scope it requires. The established read-only approval/audit boundary remains in force, and provider access is bound to the exact opaque Connection selected before refresh or provider egress. Unknown capability, provider mismatch, missing Connection, scope denial, unhealthy state, malformed search input, identity or selection change, and unavailable encrypted state fail closed. Alternative native/MCP implementations, provider routes, persisted discovery evidence, and mission/routine consumption remain planned.
