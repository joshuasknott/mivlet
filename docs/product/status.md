...
- Cloud/team backend (**Wave 2D repo-local foundation, invitation acceptance, and read-only member roster implemented; live multi-member journey open**): Convex is the canonical hosted control plane for Fable-owned users, identity links, workspaces, memberships, direct-inbox invitations, roles, devices, and shared-project authorization. Authenticated handlers implement invitation acceptance/revocation/expiry, role and status transitions, terminal removal, last-owner protection, and device revocation. Bootstrap caches only bounded display claims from the validated session, masks verified email before storage, clears withdrawn hints, and keeps every profile field display-only. The roster joins only active internal accounts with active or paused memberships; removed and inactive accounts do not disclose stale profiles or appear to retain access. Fixed native invitation and roster commands bind hosted calls to one opaque current-account generation, validate exact workspace and current-member scope, and reject malformed, duplicate, oversized, private-metadata, and profile-spoofing responses. Response loss, account switching at the final commit boundary, restart, refresh failure, unavailable invitation workspaces, Strict Mode lifecycle probes, same-tick duplicate clicks, and stale UI completion are covered. Workspace Settings resets by account and workspace, shows an honest desktop-only inbox plus a calm read-only `People with access` list, renders no member IDs, and exposes no internal-user-ID creation form. Shared-project mutations derive member/device authority, recompute canonical fingerprints, enforce exact idempotent replay and optimistic revisions, and append one immutable change snapshot per accepted workspace revision. The native sync boundary keeps hosted credentials out of React, stores an encrypted offline outbox and shared mirror, rechecks current authority around network calls, applies contiguous deltas atomically, preserves conflict shadows and durable tombstones, and fails closed when legacy hosted history needs backfill. Deployment, realtime desktop consumption, safe invitation creation, member mutation controls, and live two-account/network validation remain open.
...
| Native voice providers, Convex collaboration | **Mixed** | Dictation still depends on the host Web Speech API. Convex now has hosted membership lifecycle, a display-only member roster, and shared-project revision history plus an explicit authenticated native encrypted cache/outbox adapter. Workspace Settings can safely list and accept direct-inbox invitations and show active or paused people without fabricating preview data; invitation creation, member mutation controls, realtime desktop consumption, deployment, and live multi-member validation remain open. |
