...
| Native voice providers, Convex collaboration | **Mixed** | Dictation still depends on the host Web Speech API. Convex now has hosted membership lifecycle and shared-project revision history plus an explicit authenticated native encrypted cache/outbox adapter. Member-management UI, realtime desktop consumption, deployment, and live multi-member validation remain open. |
