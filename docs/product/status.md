...
- Account and workspace (**Wave 1A implementation complete; live validation open**): accepted ADRs and `@fable/protocol` contracts govern identity and tenancy. Clerk supplies authentication facts only; Convex owns stable internal users, identity links, workspaces, memberships, invitations, devices, and hosted authorization. Encrypted SQLite schema v13 stores the per-account workspace directory, active selection, device mirror, workspace-scoped sync state, durable conversations, and local project lifecycle. Native workspace commands fail closed without a current authenticated internal user; portable archives are bound to the active authorized workspace. The native adapter bootstraps an initial workspace idempotently, reconciles and creates workspaces, switches isolated runtime scope, and revokes account devices without exposing bearer tokens or arbitrary hosted calls to React.
...
- Optional projects (**Wave 2A lifecycle and placement implemented; project context panels open**): canonical Project contracts define local/private and shared authority, active/archive/delete lifecycle, optimistic revisions, and bounded encrypted title, description, and instructions. SQLite schema v13 preserves legacy projects and adds member-private ownership plus tombstones. Native commands derive the workspace, internal user, and member from the authenticated active context; the renderer cannot select them. The desktop shell now creates, renames, archives, restores, and confirms deletion; creates chats inside projects; moves chats into or out of a project; groups project chats separately; and keeps standalone workspace chats first-class. Deletion detaches threads, knowledge, memory, schedules, workflow definitions/runs, and pinned context instead of erasing workspace-owned records. Cross-workspace/member access, stale revisions, migration, detachment, and tombstone anti-resurrection have focused coverage. Broader optional project goals, knowledge, Connections, missions, routines, artifacts, and activity surfaces remain open.
...
| Local optional projects | **Finished for lifecycle and thread placement** | Encrypted member-private projects support create, rename, archive, restore, deletion, and optional thread association through authenticated Tauri commands. Broader project context panels remain Wave 2A work. |
