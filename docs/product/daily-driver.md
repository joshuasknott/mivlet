...
| P0 | Useful deliverables | Native bounded DOCX and formula-bearing XLSX generation now precedes the existing structural validation and publication boundary. Unit evidence verifies passive packages and an exact cached SUM result; packaged conversation rendering/publication/opening and richer document features remain unverified | Generate, validate, preview/open and export PDF, DOCX, XLSX and slides; verify layout and formulas, not just file existence |
...
| P1 | Coding workflow | Native ZIP import excludes common credential/dependency files and supports bounded reads and explicit text edits. It is not connected to the hosted process workspace, so Git initialization, builds, tests and verified diffs are explicitly unavailable for that snapshot. A configured cloud computer alone does not transfer the repository | Add an approved repository transfer into the isolated hosted workspace, then edit, test, inspect diffs, create a PR and preserve unrelated work without exposing the host shell |
| P1 | Plugin completeness | Current marketplace handles authenticated app integrations; skills live separately; Computer Use appears as a built-in plugin with the ordinary card, detail modal and native enable/disable, and readiness stays distinct from enablement | Install/enable/disable, health, permissions, skill loading and tool discovery work consistently for apps and built-ins |
