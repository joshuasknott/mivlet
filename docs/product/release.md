...
- The desktop composer exposes a bounded `/mission` entry for two to six independent tasks. It creates an encrypted Plan, pins the selected authorized provider/model route without fallback, runs one native provider worker per task, persists the command and review bundle in the source conversation, supports native cancellation and restart recovery, and requires evidence-bound human review. It does not infer dependencies, joins, synthesis, tools, effects, grants, or artifact handoff. Packaged-app and live-provider observation remain release gates.
