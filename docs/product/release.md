...
- The desktop composer exposes a bounded `/mission` entry for two to six tasks. Ordinary bullets remain independent; an explicit final `all: …` or `any: …` line declares one continuation over their immutable outputs. It creates an encrypted Plan, persists the join before outcomes exist, pins the selected authorized provider/model route without fallback, runs native provider workers through the reusable graph path, and has Rust rebuild the downstream objective from exact encrypted output receipts before egress. It persists the command and review bundle in the source conversation, supports native cancellation and restart recovery, and requires evidence-bound human review. A successful reviewed result materializes every required final deliverable as a deterministic immutable accepted Artifact in the same terminal transaction; replay revalidates its encrypted provider receipt, signed-in human evidence, selected Plan, private owner, and source conversation, while partial, failed, and cancelled results create none. It does not infer additional graph edges, tools, effects, grants, or cross-Mission handoff authority. Arbitrary multi-stage graphs, packaged-app observation, and live-provider observation remain release gates.
