 ...
 * - unexpected assets (extra chunks like unlisted Depts or future do not cause violations)
...
test("logicalChunkId ignores hashed suffixes and handles real hyphenated Vite names", () => {
  ...
  assert.equal(logicalChunkId("WarningCircle.es-yeqg3xr8.js"), "WarningCircle.es");
});
...
test("checkBudget passes within ceilings", async () => {
  ...
  const summary = {
    totalJsCss: { rawBytes: budget.observedBuild.totalJsCss.rawBytes, gzipBytes: budget.observedBuild.totalJsCss.gzipBytes },
    ...
    routeChunks: Object.fromEntries(
      Object.entries(budget.observedBuild.routeChunks).map(([chunkId, value]) => [
        chunkId,
...
test("checkBudget fails on material regression", async () => {
  ...
  const summary = {
    ...
    initialEntryJs: { rawBytes: budget.ceilings.initialEntryJs.rawBytes + 1024 },
    routeChunks: {
      SchedulesPage: { rawBytes: budget.ceilings.routeChunks.SchedulesPage.rawBytes + 1024 }
    }
  };

  const violations = checkBudget(summary, budget);
  assert.ok(violations.some((violation) => violation.label === "totalJsCss.raw"));
  assert.ok(violations.some((violation) => violation.label === "routeChunks.SchedulesPage.raw"));
});
...
test("loadBudget throws on malformed configuration (invalid JSON)", async () => {
  ...
  await writeFile(badBudgetPath, '{ "ceilings": { "totalJsCss": { "rawBytes": 1 } } '); // truncated/invalid
  await assert.rejects(
...
test("collectAssets normalizes paths to POSIX separators on Windows and mixed inputs", async () => {
  ...
  for (const a of assets) {
    assert.ok(!a.path.includes("\\"), `path should not contain backslash: ${a.path}`);
    assert.ok(a.path.includes("/"), `path should contain forward slash: ${a.path}`);
  }
...
test("checkBudget passes on exact ceiling match (threshold boundary) for all metrics", async () => {
  ...
  const summary = {
    totalJsCss: { rawBytes: c.totalJsCss.rawBytes, gzipBytes: c.totalJsCss.gzipBytes },
    css: { rawBytes: c.css.rawBytes },
    initialEntryJs: { rawBytes: c.initialEntryJs.rawBytes },
    routeChunks: Object.fromEntries(
      Object.entries(c.routeChunks).map(([chunkId, value]) => [chunkId, { rawBytes: value.rawBytes }])
    )
  };
  assert.deepEqual(checkBudget(summary, budget), []);
});
...
test("checkBudget fails when any metric exceeds its ceiling by exactly 1 byte", async () => {
  ...
  const overTotal = {
    totalJsCss: { rawBytes: c.totalJsCss.rawBytes + 1, gzipBytes: c.totalJsCss.gzipBytes },
    css: { rawBytes: c.css.rawBytes },
    initialEntryJs: { rawBytes: c.initialEntryJs.rawBytes },
    routeChunks: baseRoute
  };
  const v1 = checkBudget(overTotal, budget);
  assert.ok(v1.some((v) => v.label === "totalJsCss.raw"), "total +1 must violate");
  ...
  const overRoute = {
    ...
    css: { rawBytes: c.css.rawBytes },
    initialEntryJs: { rawBytes: c.initialEntryJs.rawBytes },
    routeChunks: { ...baseRoute, SchedulesPage: { rawBytes: c.routeChunks.SchedulesPage.rawBytes + 1 } }
  };
  const v2 = checkBudget(overRoute, budget);
  assert.ok(v2.some((v) => v.label === "routeChunks.SchedulesPage.raw"), "route +1 must violate");
});
...
test("checkBudget reports violation for missing budgeted route chunk", async () => {
  ...
  const violations = checkBudget(summary, budget);
  assert.ok(violations.some((v) => v.message.includes("expected lazy route chunk missing")));
});

test("checkBudget ignores unexpected extra assets (e.g. unlisted DepartmentsPage) when all budgeted items are within limits", async () => {
  const budget = await loadBudget();
  const summary = {
    totalJsCss: { rawBytes: budget.observedBuild.totalJsCss.rawBytes, gzipBytes: budget.observedBuild.totalJsCss.gzipBytes },
    css: { rawBytes: budget.observedBuild.css.rawBytes },
    ...
    routeChunks: {
      ...
      // extra unlisted chunk (like DepartmentsPage today)
      DepartmentsPage: { rawBytes: 669 },
  ...
  assert.ok(!violations.some((v) => /DepartmentsPage|SomeFuturePage/.test(v.message || "")));
});
...
test("summarizeBundle filters react-vendor/vendor/index and collects hyphenated route chunks (real names)", () => {
  ...
  const assets = [
    { fileName: "index-BfHRN6kf.js", path: "apps/desktop/dist/assets/index-BfHRN6kf.js", type: "js", bytes: 100, gzipBytes: 30 },
    { fileName: "react-vendor-DTgtZFgi.js", path: "apps/desktop/dist/assets/react-vendor-DTgtZFgi.js", type: "js", bytes: 200, gzipBytes: 60 },
    { fileName: "SchedulesPage-BD-qQMfd.js", path: "apps/desktop/dist/assets/SchedulesPage-BD-qQMfd.js", type: "js", bytes: 50, gzipBytes: 15 },
    { fileName: "DepartmentsPage-D4kAx-IW.js", path: "apps/desktop/dist/assets/DepartmentsPage-D4kAx-IW.js", type: "js", bytes: 10, gzipBytes: 5 },
    { fileName: "vendor-BSD_XLgc.js", path: ".../vendor-BSD_XLgc.js", type: "js", bytes: 20, gzipBytes: 7 },
    { fileName: "index-abc.css", path: ".../index-abc.css", type: "css", bytes: 30, gzipBytes: 10 },
  ];
  const summary = summarizeBundle(assets);
  assert.equal(summary.initialEntryJs && summary.initialEntryJs.rawBytes, 100);
  assert.ok(!("react-vendor" in summary.routeChunks), "react-vendor must be filtered");
  ...
  assert.equal(summary.routeChunks.DepartmentsPage.rawBytes, 10);
});
